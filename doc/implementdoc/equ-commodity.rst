...
balance function, summed over all modeled time steps :math:`t \in T_m`.
...
the parameter :math:`\overline{L}_{CO_{2}}` is set to ``inf``.
...
:math:`\overline{\overline{L}}_{CO_{2}}` is set to ``inf``.
