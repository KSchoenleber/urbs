...
new transmission capacity :math:`\hat{\kappa}_{yaf}`.

In script ``model.py`` the constraint transmission capacity rule is defined and
...
In script ``model.py`` the constraint transmission output rule is defined and
calculated by the following code fragment:
::
...
In script ``model.py`` the constraint transmission input by capacity rule is
...
:math:`\overline{K}_{yaf}`.

In script ``model.py`` the constraint transmission capacity limit rule is
defined and calculated by the following code fragment:
::
...
the complementary outgoing arc :math:`a'` between two sites must be equal.

In script ``model.py`` the constraint transmission symmetry rule is defined and
calculated by the following code fragment:
::
